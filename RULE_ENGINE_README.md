...
- ✅ **灵活配置** - 通过数据库或代码配置规则
- ✅ **执行日志** - 记录规则执行历史和性能指标
- ✅ **JSON配置** - 从JSON文件/字符串加载规则和流程配置
- ✅ **优先级排序** - 支持基于优先级的规则执行顺序
- ✅ **规则间结果传递** - 自动传递规则执行结果到后续规则
- ✅ **动态SQL** - 支持动态WHERE、ORDER BY、LIMIT拼接

## 新功能文档 (New Features Documentation)

详细的新功能使用指南请参考：
- **[JSON配置和动态功能指南](JSON_FEATURES_GUIDE.md)** - 包含JSON配置、优先级排序、动态SQL的完整使用说明和示例
